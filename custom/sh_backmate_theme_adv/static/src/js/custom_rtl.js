		...
		$(document).on("click", ".dropdown-menu a.dropdown-toggle", function () {
			...
			if ($(this).next().hasClass('show_ul')) {
				...
				if ($(this).next('.dropdown-menu').parents('.dropdown-header').length == 1) {
					...
					$(this).next('.dropdown-menu').parents('.sh_dropdown_div').children('span').removeClass('sh_dropdown');
					$(this).next('.dropdown-menu').parents('.sh_dropdown_div').children('span').removeClass('active');
				}
			}
