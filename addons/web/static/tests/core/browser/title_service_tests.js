...
QUnit.test("simple title", async (assert) => {
    assert.expect(1);
    env.services.title.setParts({ zopenerp: "Ciphlex" });
    assert.strictEqual(env.services.title.current, "Ciphlex");
});

QUnit.test("add title part", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Ciphlex", chat: null });
    assert.strictEqual(env.services.title.current, "Ciphlex");
    env.services.title.setParts({ action: "Import" });
    assert.strictEqual(env.services.title.current, "Ciphlex - Import");
});
...
QUnit.test("all at once", async (assert) => {
    assert.expect(2);
    env.services.title.setParts({ zopenerp: "Ciphlex", action: "Import" });
    assert.strictEqual(env.services.title.current, "Ciphlex - Import");
...
QUnit.test("get title parts", async (assert) => {
    ...
    assert.deepEqual(parts, { zopenerp: "Ciphlex", action: "Import" });
    parts.action = "Export";
    assert.strictEqual(env.services.title.current, "Ciphlex - Import"); // parts is a copy!
});
