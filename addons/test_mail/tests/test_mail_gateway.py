...
class TestMailgateway(TestMailCommon):
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not ciphlexai:
        ...
        ciphlexai.active = True
        ciphlexai.email = 'ciphlexai@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, ciphlexai.email_formatted, 'groups@test.com',
                subject='Ciphlexai Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, ciphlexai)
        self.assertEqual(record4.message_ids[0].email_from, ciphlexai.email_formatted)
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: ciphlexai -> no follower')
