...
@tagged("ciphlexai")
class TestCiphlexai(TestMailCommon, TestRecipients):

    @classmethod
    def setUpClass(cls):
        super(TestCiphlexai, cls).setUpClass()
        cls.test_record = cls.env['mail.test.simple'].with_context(cls._test_context).create({'name': 'Test', 'email_from': 'ignasse@example.com'})

        cls.ciphlexai = cls.env.ref("base.partner_root")
        ...
        cls.ciphlexai_ping_body = '<a href="http://odoo.com/web#model=res.partner&amp;id=%s" class="o_mail_redirect" data-oe-id="%s" data-oe-model="res.partner" target="_blank">@CiphlexAi</a>' % (cls.ciphlexai.id, cls.ciphlexai.id)
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('odoo.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_ciphlexai()
        ciphlexai = self.env.ref("base.partner_root")
        ciphlexai_in_fetch_listeners = self.env['mail.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', ciphlexai.id)])
        self.assertEqual(len(ciphlexai_in_fetch_listeners), 1, 'ciphlexai should appear only once in channel_fetch_listeners')

    @mute_logger('odoo.addons.mail.models.mail_mail')
    def test_ciphlexai_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.ciphlexai_ping_body, 'partner_ids': [self.ciphlexai.id, self.user_admin.partner_id.id]})
        ...
        with patch('random.choice', lambda x: x[0]):
            self.assertNextMessage(
                ...
                sender=self.ciphlexai,
                answer=False
            )
        # Ciphlexai should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.ciphlexai, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last ciphlexai message instead
            sender=self.ciphlexai,
            answer=("@CiphlexAi",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.ciphlexai_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.ciphlexai_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('odoo.addons.mail.models.mail_mail')
    def test_ciphlexai_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @ciphlexai right now", 'partner_ids': []})
        self.assertNextMessage(
